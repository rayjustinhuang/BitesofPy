...
def _convert_struct_time_to_dt(stime):
    ...
    -> date(2016, 12, 28)
    """
    date_object = date.fromtimestamp(time.mktime(stime))
    return date_object
    pass
...
def get_feed_entries(feed=FEED):
    ...
    for item in d.entries:
        ...
        timestruct = item.published_parsed
        date = _convert_struct_time_to_dt(timestruct)
        
        title = item.title
        link = item.link
        tags = [i.term.lower() for i in item.tags]
        
        entries_list.append(Entry(date, title, link, tags))
