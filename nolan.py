...
def get_movies():
    ...
    return [row.attrib['title'] for row in get_tree()]
    pass
...
def get_movie_longest_runtime():
    ...
    movie_lengths = [(row.attrib['title'], int(row.attrib['runtime'].split()[0])) for row in get_tree()]
    return sorted(movie_lengths, key = lambda x: x[1], reverse=True)[0][0]
    pass