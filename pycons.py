...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    useful_data = soup.find_all('script')
    
    events_raw = [i.text.strip() for i in useful_data]
    
    pycon_list = []
    
    for event in events_raw:
        source = json.loads(event)
        if 'PyCon' in source['name']:
            start = parse(source['startDate'])
            end = parse(source['endDate'])
            pycon_list.append(PyCon(source['name'], source['location']['address']['addressLocality'], source['location']['address']['addressCountry'], start, end, source['url']))
...
def filter_pycons(pycons: List[PyCon],
    ...
    that continent.
    """
    year_filter = filter(lambda x: x.start_date.year == year, pycons)
    continent_filter = filter(lambda y: get_continent(y.country) == continent, year_filter)
    
    return [i for i in continent_filter]
    pass